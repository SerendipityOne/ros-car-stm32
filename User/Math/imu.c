...
void GetAngle(const MPU_t* mpu, Ange_t* ange, float dt) {
  ...
  //角速度融合加速度积分补偿值
  Gyro.x = mpu->gyroX * Gyro_Gr + KpDef * AccGravity.x + GyroIntegError.x;  //弧度制
  Gyro.y = mpu->gyroY * Gyro_Gr + KpDef * AccGravity.y + GyroIntegError.y;
  Gyro.z = mpu->gyroZ * Gyro_Gr + KpDef * AccGravity.z + GyroIntegError.z;
  // 一阶龙格库塔法, 更新四元数
...
#ifdef YAW_GYRO
    *(
        float*)ange = atan2f(2 * NumQ.q1 * NumQ.q2 + 2 * NumQ.q0 * NumQ.q3, 1 - 2 * NumQ.q2 * NumQ.q2 - 2 * NumQ.q3 * NumQ.q3) * RtA;  //yaw
#else
    float yaw_G = mpu->gyroZ * Gyro_G;
